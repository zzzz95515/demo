...
public class CalculateP0Service {
    ...
    public Double calculateP0(Long id, List<LocalDate> dateTime, Integer var){
        ...
        if (dateTime.get(0)==null){
            ...
            its_after0 = predictedItsRepo.findByEquipmentAndDateAfterAndAndPlanNumber(equipment,dateTime.get(0),var).getIts();
            its_before0 = predictedItsRepo.findByEquipmentAndDateBeforeAndAndPlanNumber(equipment,dateTime.get(0),var).getIts();
        }
        Double p0=0.0;
        Double p=0.0;
        if (equipment!=null){
            Double its_after = predictedItsRepo.findByEquipmentAndDateAfterAndAndPlanNumber(equipment,dateTime.get(1),var).getIts();
            Double its_before = predictedItsRepo.findByEquipmentAndDateBeforeAndAndPlanNumber(equipment,dateTime.get(1),var).getIts();
            if (its_after!=null && its_before!=null) {
                p=1-(1-its_after/100)*its_before/its_after;
            }
            else {
            ...
            Double predictedP=1 - (1-its_after0/100)*its_before0/its_after0*Math.exp(-alpha*24*ChronoUnit.DAYS.between(dateTime.get(0),dateTime.get(1)));
            p0= p-predictedP;
        }
        else{
    ...
    private Map<LocalDate,Double> calculation (EquipmentEnt equipment, LocalDate date1, LocalDate date2,Integer var, Boolean flag){
        ...
        Double its_before;
        if (flag){
            its_after = predictedItsRepo.findByEquipmentAndDateAfterAndAndPlanNumber(equipment,date1,var).getIts();
            its_before =predictedItsRepo.findByEquipmentAndDateBeforeAndAndPlanNumber(equipment,date1,var).getIts();
        }else {
